...
(defn circumference []
  (fn [mesh]
    (let [mesh (mp/assoc-face-circ-map mesh)
...
(defn normal-mod1-rgb []
  (fn [mesh]
    (let [fc (fn [mesh face]
               (let [[r g b] (mapv #(mod % 1) (mf/normal face))
                     alpha 1.0]
                 [r g b alpha]))]
      [mesh fc])))
...
(defn normal-abs-cie1931 []
  (fn [mesh]
    (let [fc (fn [mesh face]
               (let [[x y z] (mapv mu/abs (mf/normal face))
                     color (col/as-rgba (col/cie1931 [x y z 1.0]))]
                 @color))]
      [mesh fc])))
...
(defn normal-mod1-sum-hue []
  (fn [mesh]
    (let [fc (fn [mesh face]
               (let [[x y z] (mapv #(mod % 1) (mf/normal face))
...
(defn blend-with-edge-neighbors [t]
  (fn [mesh]
    (let [mesh (mp/assoc-edge-faces-map mesh)]
      [mesh (partial blend-with-neighbors mf/edge-neighbors t)])))

(defn blend-with-vertex-neighbors [t]
  (fn [mesh]
    (let [mesh (mp/assoc-vert-npfs-map mesh)]
      [mesh (partial blend-with-neighbors mf/vert-neighbors t)])))

(defn blend-with-vertex-only-neighbors [t]
  (fn [mesh]
    (let [mesh (mp/assoc-edge-faces-map mesh)
          mesh (mp/assoc-vert-npfs-map mesh)]
      [mesh (partial blend-with-neighbors mf/vert-only-neighbors t)])))
...
(defn kitchen-sink []
  (fn [mesh]
    (let [mesh-centroid (mp/centroid mesh)
          ...
          fc (fn [mesh face]
               (let [[x y z] (mapv mu/abs (mf/normal face))
                     [nx ny nz] (mapv #(mod % 1) (mf/normal face))
